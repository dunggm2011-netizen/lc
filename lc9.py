# =====================================================================
# LC79 TELE68 BOT v3 — 31 MODEL + SKIP MODE + IMAGE RESULT
# API: wtxmd52.tele68.com/v1/txmd5/sessions
# Admin: t.me/+LaaT-vDzLo02MDJl
# Tool: TOOL THANH DUY
# =====================================================================
...
def run_engine(sessions):
    ...
    if not f or f["n"] < 10:
        return {
            "prediction": "TAI", "confidence": 0.5, "skip": True,
            "skip_reason": "Chưa đủ 10 phiên", "regime": "COLD_START",
            "model_count": 0, "reasons": [],
        }
    ...
    if not models:
        return {
            "prediction": "TAI", "confidence": 0.5, "skip": True,
            "skip_reason": "Không có model active", "regime": "COLD_START",
            "model_count": 0, "reasons": [],
        }
    ...
    if totalW == 0:
        return {
            "prediction": "TAI", "confidence": 0.5, "skip": True,
            "skip_reason": "Total weight = 0", "regime": "COLD_START",
            "model_count": len(models), "reasons": [],
        }
    ...
    skip = False
    skip_reason = None
    if conf < ADAPTIVE_FLOOR:
        skip = True; skip_reason = f"Conf {round(conf*100)}% < {round(ADAPTIVE_FLOOR*100)}%"
    if f["ent"] > 0.98:
        skip = True; skip_reason = f"Cầu loạn (entropy {f['ent']:.2f})"
    if f["switchRate"] > 0.85 and f["n"] < 15:
        skip = True; skip_reason = f"Cầu 1-1 nhiễu (switch {f['switchRate']:.2f})"
    if len(models) < 5:
        skip = True; skip_reason = f"Chỉ {len(models)} model active"
    if agree < 0.55:
        skip = True; skip_reason = f"Agree thấp ({round(agree*100)}%)"
    if regime == "HIGH_ENTROPY":
        skip = True; skip_reason = "Regime HIGH_ENTROPY"
    ...
    return {
        ...
        "reasons": reasons,
        "entropy": round(f["ent"], 3),
        "switch_rate": round(f["switchRate"], 3),
    }
...
def draw_dice(draw, cx, cy, size, value, angle=0):
    half = size // 2
    draw.rounded_rectangle([cx-half, cy-half, cx+half, cy+half],
                            radius=12, fill=(255,255,255), outline=(0,0,0), width=2)
    dot_r = size // 12
    ...
    for (dx, dy) in positions:
        ...
        px = cx + int(rx * size * 0.3)
        py = cy + int(ry * size * 0.3)
        draw.ellipse([px-dot_r, py-dot_r, px+dot_r, py+dot_r], fill=(230, 50, 50))
...
def generate_result_image(prediction, actual, dice, total, session_result,
    ...
    for i in range(0, W, 60):
        for j in range(0, H, 60):
            draw.ellipse([i-2, j-2, i+2, j+2], fill=(40, 40, 60))

    # Top card
    draw.rectangle([30, 20, W-30, 500], outline=(255, 215, 0), width=4)
    draw.rectangle([34, 24, W-34, 496], fill=(0, 130, 200))

    label = "TÀI" if actual == "TAI" else "XỈU"
    font_big = load_font(72)
    draw.text((60, 40), label, fill=(255, 220, 0), font=font_big)

    font_huge = load_font(90)
    total_str = str(total) if total else "--"
    bbox = draw.textbbox((0,0), total_str, font=font_huge)
    tw = bbox[2] - bbox[0]
    draw.text((W - 60 - tw, 30), total_str, fill=(255, 255, 255), font=font_huge)
    ...
    if dice and len(dice) >= 3:
        d_size = 90
        draw_dice(draw, cx-60, cy-50, d_size, dice[0], angle=-15)
        draw_dice(draw, cx+60, cy-50, d_size, dice[1], angle=10)
        draw_dice(draw, cx, cy+50, d_size, dice[2], angle=-5)

    font_title = load_font(48)
    bbox = draw.textbbox((0,0), TOOL_NAME, font=font_title)
    tw = bbox[2] - bbox[0]
    draw.text(((W-tw)//2, 445), TOOL_NAME, fill=(255, 220, 0), font=font_title)

    # Bottom info
    draw.rectangle([30, 520, W-30, H-30], fill=(15, 15, 30), outline=(255, 215, 0), width=2)
    font_info = load_font(36)

    y = 550
    draw.text((60, y), f"🎰 AI 68GB", fill=(0, 255, 200), font=font_info)
    draw.text((450, y), f"— {TOOL_NAME}", fill=(255, 255, 255), font=font_info)

    y += 70
    draw.text((60, y), f"🎯 Bàn Xanh", fill=(255, 100, 100), font=font_info)
    draw.text((400, y), f"📅 Phiên #{session_result}", fill=(255, 220, 0), font=font_info)

    y += 70
    actual_txt = "Tài" if actual == "TAI" else "Xỉu"
    draw.text((60, y), f"🎲 Kết quả:", fill=(200, 200, 200), font=font_info)
    draw.text((350, y), f"✅ {actual_txt}", fill=(100, 255, 100), font=font_info)

    y += 70
    draw.line([60, y, W-60, y], fill=(255, 140, 0), width=3)
    y += 30

    draw.text((60, y), f"🆕 Bàn Xanh", fill=(255, 100, 100), font=font_info)
    draw.text((400, y), f"📅 Phiên mới #{session_next}", fill=(255, 220, 0), font=font_info)

    y += 70
    pred_txt = "Tài" if prediction == "TAI" else "Xỉu"
    draw.text((60, y), f"🎯 Dự đoán:", fill=(200, 200, 200), font=font_info)
    pred_color = (255, 100, 100) if prediction == "TAI" else (100, 150, 255)
    draw.text((400, y), f"☑️ {pred_txt}", fill=pred_color, font=font_info)

    y += 70
    draw.text((60, y), f"⚡ Độ tin cậy:", fill=(200, 200, 200), font=font_info)
    draw.text((450, y), f"{confidence}%", fill=(0, 255, 200), font=font_info)

    y += 70
    draw.text((60, y), f"💬 Nhóm hỗ trợ:", fill=(200, 200, 200), font=font_info)
    draw.text((60, y + 45), GROUP_LINK_TEXT, fill=(100, 180, 255), font=font_info)
...
def fmt_prediction(session_id, engine_out):
    ...
    if engine_out.get("skip"):
        return (
            f"⚠️ <b>KHÔNG ĐÁNH — PHIÊN #{session_id}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"🚫 Lý do: <b>{engine_out.get('skip_reason', 'Cầu mờ')}</b>\n"
            ...
            if data and "list" in data and len(data["list"]) > 0:
                ...
                if current_id != state["last_session_id"]:
                    ...
                    if np is not None and np.get("target_id") == current_id:
                        if np.get("skip"):
                            state["skip_count"] += 1
                            log_entry = f"#{current_id} | SKIP | Ra {actual_result}"
                    ...
                    state["next_prediction"] = {
                        ...
                        "skip": engine_out.get("skip", False),
                    }
                    ...
                    if CHAT_ID:
                        ...
                        try:
                            ...
                            tag = "SKIP" if engine_out.get("skip") else engine_out["prediction"]
                            log.info(f"Sent #{target_session_id}: {tag} ({engine_out['confidence']*100:.1f}%)")
                        except Exception as e:
