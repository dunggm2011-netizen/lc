# =====================================================================
# LC79 TELE68 BOT v7 — ẢNH + LINK NHÓM QUA TIN NHẮN + REASON 12L
...
def engine_v2(sessions):
    """Trả về (suggestion, confidence, reason_chuỗi_12_layer)."""
    ...
    # Mặc định mọi layer = 0
    L = {
        "L1_antibreak": 0, "L2_11": 0, "L3_m3": 0, "L4_m4": 0,
        "L5_point": 0, "L6_f10": 0, "L7_f20": 0, "L8_mom": 0,
        "L9_vol": 0, "L10_ent": 0, "L11_stk": 0, "L12_r5": 0,
    }

    # L1 + L2: nếu khớp → return ngay
    if f["cur"]["len"] >= 3:
        L["L1_antibreak"] = 1
        reason = " | ".join(f"{k}:{v:+g}" for k, v in L.items())
        return (last_res, 88.5, reason)
    ...
    L["L8_mom"] = momentum_score * 0.4

    # L9: Volatility
    volatility_score = 0
    if f["switchRate"] > 0.75:
        volatility_score = -10
    L["L9_vol"] = volatility_score * 0.3
    ...
    L["L10_ent"] = entropy_score * 0.5

    # L11: Streak ratio
    streak_score = 0
    if f["cur"]["len"] == 2:
        streak_score = 5 if f["cur"]["val"] == "TAI" else -5
    L["L11_stk"] = streak_score * 0.4
...
def generate_prediction_image(prediction, session_next, confidence, dice, total):
    ...
    draw_base_card(draw, W, H, label, total, dice, TOOL_NAME)
    ...
    draw.text((180, y), f"Phien #{session_next}", fill=(255, 220, 0), font=font_info)
    ...
    draw.text((180, y), f"-> {pred_txt}", fill=pred_color, font=font_info)
...
def generate_result_image(actual, session_result, dice, total):
    ...
    draw.text((180, y), f"-> {actual_txt}", fill=actual_color, font=font_info)
...
# ==================== SEND ====================
def send_prediction_image(prediction, session_next, confidence, reason, dice, total):
    """Gửi ảnh dự đoán + caption có link nhóm + reason 12 layer."""
    try:
        img_buf = generate_prediction_image(prediction, session_next, confidence, dice, total)
        arrow = "🟢 TÀI" if prediction == "TAI" else "🔵 XỈU"
        caption = (
            f"🎯 <b>DỰ ĐOÁN PHIÊN #{session_next}</b>\n"
            f"Kết quả: <b>{arrow}</b>\n"
...
def send_result_image(actual, session_result, dice, total):
    """Gửi ảnh kết quả + caption có link nhóm."""
    try:
        ...
        arrow = "🟢 TÀI" if actual == "TAI" else "🔵 XỈU"
        caption = (
            f"📍 <b>KẾT QUẢ PHIÊN #{session_result}</b>\n"
            f"Kết quả: <b>{arrow}</b>\n"
                ...
                if current_id != state["last_session_id"]:
                    ...
                    if CHAT_ID:
                        ...
                        send_prediction_image(
                            suggestion, target_session_id, confidence, reason,
...
def cmd_start(m):
    bot.reply_to(
        m,
        f"🤖 <b>LC79 Bot v7 — 12 Layer, chỉ ảnh</b>\n\n"
        f"Tool: {TOOL_NAME}\n"
...
def cmd_predict(m):
    ...
    send_prediction_image(suggestion, current_id + 1, confidence, reason, dice, total)
