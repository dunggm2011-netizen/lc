# =====================================================================
# LC79 TELE68 BOT v8 — DỰ ĐOÁN TRÊN ẢNH, CAPTION CHỈ LINK + KQ
...
def engine_v2(sessions):
    """Trả về (suggestion, confidence, reason)."""
...
# ==================== IMAGE 1: PREDICTION (có dự đoán trên ảnh) ====================
def generate_prediction_image(prediction, session_next, confidence, dice, total):
    """Ảnh dự đoán — hiển thị dự đoán + độ tin cậy ngay trên ảnh."""
    W, H = 600, 600
    img = Image.new("RGB", (W, H), (0, 0, 0))
    draw = ImageDraw.Draw(img)

    # Label = DỰ ĐOÁN (không phải kết quả)
...
def send_result_image(actual, session_result, dice, total, correct, predicted):
    """Gửi ảnh kết quả — caption có link nhóm + đúng/sai."""
    try:
        ...
        try:
            ...
            if data and "list" in data and len(data["list"]) > 0:
                ...
                if current_id != state["last_session_id"]:
                    ...
                    # 1. Chấm điểm phiên cũ — gửi ảnh kết quả + caption đúng/sai
                    ...
                    # 3. Gửi ảnh dự đoán — caption chỉ link nhóm
                    if CHAT_ID:
